...
for (dir in dirs) {
    ...
    for (file in files) {
        ...
        all_data_complete <- bind_rows(all_data_complete, data)
    }
}
weeks <- c("2024_45", "2024_46", "2024_47", "2024_48", "2024_49", "2024_50", "2024_51", "2024_52", "2025_01", "2025_02", "2025_03", "2025_04", "2025_05", "2025_06", "2025_07", "2025_08", "2025_09", "2025_10", "2025_11", "2025_12", "2025_13", "2025_14", "2025_15", "2025_16")

for (w in weeks) {
    limit <- w
    all_data <- all_data_complete %>%
        filter(
    ...
    labels_order <- c(
        paste0("2024-", c(40:52)),
        paste0("2025-", sprintf("%02d", c(1:25)))
    )
    ...
    ensemble_comuni <- ggplot() +
        geom_ribbon(data = comunipd, mapping = aes(x = year_week, ymin = `0.025`, ymax = `0.975`, group = "Force of Infection", fill = "Force of Infection"), alpha = 0.2) +
        geom_ribbon(data = comunipd, mapping = aes(x = year_week, ymin = `0.05`, ymax = `0.95`, group = "Force of Infection", fill = "Force of Infection"), alpha = 0.4) +
        geom_ribbon(data = comunipd, mapping = aes(x = year_week, ymin = `0.25`, ymax = `0.75`, group = "Force of Infection", fill = "Force of Infection"), alpha = 1) +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.25`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "solid") +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.75`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "solid") +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.95`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "dashed") +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.05`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "dashed") +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.025`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "dotted") +
        geom_line(data = ensemble, mapping = aes(x = year_week, y = `0.975`, group = "Ensemble", color = "Ensemble"), size = 1, linetype = "dotted") +
...
ggsave(paste0("previsioni/output/compact_plot_", last_incidence$year_week, ".png"),
    width = 13, height = 8, dpi = 300
)
# ggsave(paste0("previsioni/output/compact_plot_latest.png"),
#     width = 13, height = 8, dpi = 300
# )
